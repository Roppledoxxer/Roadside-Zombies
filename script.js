...
const lootTypes = [
  { type: "boot", icon: "👢", name: "BOOT", value: 1, weight: 55 },
  { type: "flesh", icon: "🥩", name: "FLESH", value: 0, weight: 30 },
...
function resetGame() {
  ...
  document.querySelectorAll(".touchBtn").forEach(btn => btn.classList.remove("pressed"));
  mobileControls.classList.toggle("hidden", controlMode !== "mobile");
...
function setControlMode(mode) {
  ...
  pcControls.classList.toggle("hiddenControlInfo", isMobile);
  mobileControlsInfo.classList.toggle("hiddenControlInfo", !isMobile);
...
canvas.addEventListener("mousedown", e => {
  if (controlMode === "pc" && e.button === 0) mouse.down = true;
});
...
function mobileShootStart(e) {
  e.preventDefault();
  if (!game.running || game.dead || controlMode !== "mobile") return;
  document.getElementById("mobileShoot").classList.add("pressed");
  mouse.down = true;
  ...
  if (target) {
    ...
    mouse.x = game.player.x + Math.cos(game.player.facing) * 400;
    mouse.y = game.player.y + Math.sin(game.player.facing) * 400;
  }
...
function mobileShootEnd(e) {
  e.preventDefault();
  document.getElementById("mobileShoot").classList.remove("pressed");
  mouse.down = false;
}

const mobileShootBtn = document.getElementById("mobileShoot");
mobileShootBtn.addEventListener("pointerdown", mobileShootStart);
window.addEventListener("pointerup", mobileShootEnd);
window.addEventListener("pointercancel", mobileShootEnd);

document.getElementById("mobileUse").addEventListener("pointerdown", e => {
  e.preventDefault();
  if (!game.running || game.dead || controlMode !== "mobile") return;

  const shopHidden = document.getElementById("shopMenu").classList.contains("hidden");
  if (shopHidden) interact();
  else closeShop();
});
...
function isInsideZone(p, z, pad = 0) {
  return p.x >= z.x - pad && p.x <= z.x + z.w + pad &&
         p.y >= z.y - pad && p.y <= z.y + z.h + pad;
}
...
function interact() {
  if (isInsideZone(game.player, sellZone, 18)) { sellInventory(); return; }
  if (isInsideZone(game.player, shopZone, 18)) { openShop(); return; }
  showToast("Move into a station and press E.");
}
...
function renderShop() {
  const items = [
    {id:"health",icon:"♥",name:"MORE HEALTH",price:20,desc:"+1 heart"},
    {id:"shotgun",icon:"🔫",name:"SHOTGUN",price:30,desc:"7-pellet spread"},
...
function sellInventory() {
  ...
  for (const stack of game.inventory) {
    earned += stack.value * stack.amount;
    count += stack.amount;
  }
  ...
  showToast(`SOLD ${count} LOOT — +$${earned}`);
  updateUI();
}
...
function buyNextUsefulItem() {
  ...
  for (const [item, cost] of options.slice(1)) {
    if (!game.ownedWeapons[item] && game.money >= cost) {
      game.money -= cost;
      game.ownedWeapons[item] = true;
      selectWeapon(item);
      showToast(`${weapons[item].name.toUpperCase()} PURCHASED`);
      updateUI();
      return;
    }
  }
...
function randomLoot() {
  const total = lootTypes.reduce((s, x) => s + x.weight, 0);
...
function addLoot(loot) {
  // Add to an existing stack first.
  const stack = game.inventory.find(item => item.type === loot.type && item.amount < MAX_STACK);

  if (stack) {
    stack.amount++;
    showToast(`${loot.name} +1 — ${stack.amount}/${MAX_STACK}`);
    updateUI();
    return true;
  }

  // No stack available: use a new inventory slot.
  ...
  showToast(`${loot.name} COLLECTED 1/${MAX_STACK}`);
  updateUI();
  return true;
}
...
function shoot() {
  ...
  const angBase = Math.atan2(mouse.y - game.player.y, mouse.x - game.player.x);
  game.player.facing = angBase;

  for (let i = 0; i < gun.pellets; i++) {
    const ang = angBase + (Math.random() - 0.5) * gun.spread;
    game.bullets.push({
      x: game.player.x + Math.cos(ang) * 18,
      y: game.player.y + Math.sin(ang) * 18,
...
function spawnZombie() {
  ...
  if (fromSide) {
    ...
    x = side < 0 ? 25 : W - 25;
    y = Math.max(MIN_WORLD_Y, game.player.y + (Math.random() - 0.5) * 750);
  } else {
    x = ROAD.x + 25 + Math.random() * (ROAD.w - 50);
...
function update(dt) {
  ...
  const speedBoost = keys["shift"] ? 1.25 : 1;
  ...
  if (game.spawnTimer <= 0) {
    const currentNear = game.zombies.length;
    if (currentNear < 22) spawnZombie();
  ...
  for (let i = game.zombies.length - 1; i >= 0; i--) {
    ...
    const vx = game.player.x - z.x;
    const vy = game.player.y - z.y;
...
function addParticle(x, y, color, life = 0.35, speed = 12) {
...
function drawWorld() {
  ...
  for (let y = 0; y < H; y += 26) {
    for (let x = 0; x < W; x += 26) {
      ...
      if (r > 0.55) {
        ctx.fillStyle = r > 0.8 ? "#465536" : "#536243";
        ctx.fillRect(x + (r * 16) | 0, y + (r * 9) | 0, 3, 2);
      }
    }
  }
  ...
  for (let y = 12; y < H; y += 80) {
    ...
    ctx.fillRect(ROAD.x + ROAD.w + 12, y + 30, 8, 18);
  }
}
...
function updateUI() {
  ...
  } else if (game.inventory.length >= MAX_SLOTS && game.inventory.every(item => item.amount >= MAX_STACK)) {
    msg = "INVENTORY FULL — RETURN TO THE HUT";
  } else if (controlMode === "mobile") {
    msg = "ARROWS TO MOVE • SHOOT TO ATTACK";
  } else {
